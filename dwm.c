...
static void     sendmon(Client*, Monitor*);
...
static void     tabmode(const Arg*);
static void     tag(const Arg*);
static void     tagmon(const Arg*);
static void     tile(Monitor*);
static void     togglebar(const Arg*);
...
static void     updatebarpos(Monitor*);
...
void cleanup(void) {
    ...
    for (m = mons; m; m = m->next) while (m->stack) unmanage(m->stack, 0);
...
void drawtab(Monitor *m) {
    ...
    if(tot_width > m->ww){ //not enough space to display the labels, they need to be truncated
        ...
        tot_width = view_info_w;
        for(i = 0; i < m->ntabs; ++i) {
            if(tot_width + (m->ntabs - i) * sorted_label_widths[i] > m->ww) break;
...
void tabmode(const Arg *arg) {
    if(arg && arg->i >= 0) selmon->showtab = arg->ui % showtab_nmodes;
...
void updatetitle(Client *c) {
    ...
    if (c->name[0] == '\0') strcpy(c->name, broken);
}
